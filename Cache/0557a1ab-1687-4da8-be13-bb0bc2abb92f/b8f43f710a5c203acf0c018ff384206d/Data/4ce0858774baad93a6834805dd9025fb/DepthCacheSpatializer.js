...
exports.DepthSpatializer = void 0;
var __selfType = requireType("./DepthCacheSpatializer");
...
const SpatializerUtils_1 = require("./SpatializerUtils");
...
let DepthSpatializer = (() => {
    let _classDecorators = [component];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = BaseScriptComponent;
    var DepthSpatializer = _classThis = class extends _classSuper {
        constructor() {
            super();
            this.camera = this.camera;
            this.debugVisualizer = this.debugVisualizer;
            this.depthCache = this.depthCache;
            this.depthPrefab = this.depthPrefab;
            this.testButton = this.testButton;
            this.mlSpatializer = this.mlSpatializer;
            this.isRequestRunning = false;
            this.detectionInstances = [];
            this.debug = this.debug;
            this.enableContinuousUpdate = this.enableContinuousUpdate;
            this.continuousUpdateInterval = this.continuousUpdateInterval;
            // Smoothing technique properties
            this.maxDetections = this.maxDetections;
            this.boundingBoxScale = this.boundingBoxScale;
            this.positionUpdateThreshold = this.positionUpdateThreshold;
            this.maxCameraRotationSpeed = this.maxCameraRotationSpeed;
            this.maxCameraMovementSpeed = this.maxCameraMovementSpeed;
            this.vertexUpdateThreshold = this.vertexUpdateThreshold;
            this.stableFramesRequired = this.stableFramesRequired;
            // Static scene properties for pre-instantiated prefabs
            this.enableStaticScene = this.enableStaticScene;
            this.lerpDuration = this.lerpDuration;
            this.vertexLerpDuration = this.vertexLerpDuration;
            this.enableRotationLerp = this.enableRotationLerp;
            this.rotationLerpDuration = this.rotationLerpDuration;
            ...
            this.preInstantiatedDetections = [];
            this.detectionStates = [];
            this.activeLerps = new Map();
        }
        __initialize() {
            super.__initialize();
            this.camera = this.camera;
        ...
        updatePosition() {
            ...
            try {
                ...
                if (depthFrameID === -1) {
                    if (this.debug) {
                        print("Failed to request depth frame - depth system may not be ready");
                    }
                    this.isRequestRunning = false;
                    return;
                }
        ...
        depthWorldPosition(cameraFrame, depthFrameID) {
         ...
         */
        processDetectionsWithStaticScene(detections, cameraFrame, depthFrameID) {
            const frameWidth = cameraFrame.getWidth();
            const frameHeight = cameraFrame.getHeight();
            for (let i = 0; i < detections.length; i++) {
                ...
                if (!worldPosition) {
                    if (this.debug) {
                        print(`Static Scene Detection ${i}: Failed to get world position for pixel (${pixelX.toFixed(1)}, ${pixelY.toFixed(1)})`);
                    }
                    continue;
                }
                ...
                if (this.debug) {
                    ...
                    print(`  World position: (${worldPosition.x.toFixed(2)}, ${worldPosition.y.toFixed(2)}, ${worldPosition.z.toFixed(2)})`);
                }
            }
            ...
            for (let i = detections.length; i < this.maxDetections; i++) {
                if (i < this.detectionStates.length && this.detectionStates[i].isActive) {
                    ...
                    if (this.detectionStates[i].fadeAlpha <= 0) {
                        ...
                        if (i < this.preInstantiatedDetections.length &&
         ...
         */
        processDetectionsWithDynamicScene(detections, cameraFrame, depthFrameID) {
            const frameWidth = cameraFrame.getWidth();
            const frameHeight = cameraFrame.getHeight();
            ...
            for (let i = 0; i < detections.length; i++) {
                ...
                if (i < this.detectionInstances.length && this.detectionInstances[i]) {
                    ...
                    if (shouldUpdateDetection.updateCenter) {
                        instance
                            .getTransform()
                            .setWorldPosition(new vec3(worldPosition.x, worldPosition.y, worldPosition.z));
                        if (this.debug) {
                ...
                this.updateDetectionContainer(instance, detection, worldPosition, boundingBoxVertices, shouldUpdateDetection.updateVertices);
                // Enable the instance after a brief delay to allow polyline to refresh
                if (!instance.enabled) {
                    const enableDelayEvent = this.createEvent("DelayedCallbackEvent");
                ...
                if (this.debug) {
                    ...
                    print(`  Center: pixel (${pixelX.toFixed(1)}, ${pixelY.toFixed(1)}) -> normalized (${centerX.toFixed(3)}, ${centerY.toFixed(3)})`);
                    print(`  World position: (${worldPosition.x.toFixed(2)}, ${worldPosition.y.toFixed(2)}, ${worldPosition.z.toFixed(2)})`);
                    print(`  Updates: center=${shouldUpdateDetection.updateCenter}, vertices=${shouldUpdateDetection.updateVertices}`);
                }
            }
        }
        /**
         ...
         */
        updateStaticSceneDetection(detectionIndex, detection, worldPosition, boundingBoxVertices) {
            // Ensure we have enough pre-instantiated detections
            if (detectionIndex >= this.preInstantiatedDetections.length) {
                if (this.debug) {
                    print(`Static Scene: Detection index ${detectionIndex} exceeds pre-instantiated detections (${this.preInstantiatedDetections.length})`);
                }
                return;
            }
            ...
            const needsPositionUpdate = !state.targetPosition ||
                state.targetPosition.distance(worldPosition) >
                    this.positionUpdateThreshold / 100; // Convert cm to meters
            const needsVertexUpdate = !state.targetVertices ||
                !(0, SpatializerUtils_1.areVerticesSimilar)(state.targetVertices, boundingBoxVertices, this.vertexUpdateThreshold);
            if (needsPositionUpdate || needsVertexUpdate) {
                ...
                this.startLerpTransition(detectionIndex, worldPosition, boundingBoxVertices, currentRotation // Keep current rotation for now
                );
                if (this.debug) {
                    print(`Static Scene Detection ${detectionIndex}: Started lerp transition`);
                    print(`  From: (${currentPosition.x.toFixed(2)}, ${currentPosition.y.toFixed(2)}, ${currentPosition.z.toFixed(2)})`);
         ...
         */
        updateDetectionContainer(detectionInstance, detection, worldPosition, boundingBoxVertices, updateVertices = true) {
            ...
            if (updateVertices &&
                ...
                for (let i = 0; i < Math.min(4, boundingBoxVertices.length); i++) {
                    if (detectionContainer.polylinePoints[i]) {
                        ...
                        if (this.debug) {
                            print(`Updated polyline point ${i} to world position: (${boundingBoxVertices[i].x.toFixed(2)}, ${boundingBoxVertices[i].y.toFixed(2)}, ${boundingBoxVertices[i].z.toFixed(2)})`);
                        }
                    }
                }
                ...
                if (detectionContainer.polyline) {
                    ...
                    const delayedRefreshEvent = this.createEvent("DelayedCallbackEvent");
         ...
         */
        resizeTrackingArrays(targetLength) {
         ...
         */
        calculateBoundingBoxVertices(detection, centerWorldPosition, cameraFrame, depthFrameID) {
            ...
            const worldVertices = [];
            if (this.debug) {
                print(`Calculating vertices for bbox [${centerX.toFixed(3)}, ${centerY.toFixed(3)}, ${width.toFixed(3)}, ${height.toFixed(3)}] with scale ${this.boundingBoxScale.toFixed(2)}`);
            ...
            for (let i = 0; i < normalizedVertices.length; i++) {
                ...
                if (this.debug) {
                    print(`Vertex ${i}: normalized (${normalizedVertex.x.toFixed(3)}, ${normalizedVertex.y.toFixed(3)}) -> pixel (${pixelPos.x.toFixed(1)}, ${pixelPos.y.toFixed(1)})`);
                }
                ...
                if (vertexWorldPosition) {
                    ...
                    if (this.debug) {
                        print(`Vertex ${i}: Got depth-based world position (${vertexWorldPosition.x.toFixed(2)}, ${vertexWorldPosition.y.toFixed(2)}, ${vertexWorldPosition.z.toFixed(2)})`);
                    }
                }
                else {
                    ...
                    const fallbackVertex = new vec3(centerWorldPosition.x + offsetX, centerWorldPosition.y + offsetY, centerWorldPosition.z);
                    worldVertices.push(fallbackVertex);
                    if (this.debug) {
                        print(`Vertex ${i}: Fallback calculation - distance: ${distanceFromCamera.toFixed(2)}, scale: ${worldScale.toFixed(4)}, offset: (${offsetX.toFixed(2)}, ${offsetY.toFixed(2)})`);
                        print(`Vertex ${i}: Fallback world position (${fallbackVertex.x.toFixed(2)}, ${fallbackVertex.y.toFixed(2)}, ${fallbackVertex.z.toFixed(2)})`);
                    }
                }
            }
            // Align vertices to form a perfect rectangle
            if (worldVertices.length === 4) {
                const alignedVertices = (0, SpatializerUtils_1.alignVerticesToRectangle)(worldVertices, this.debug);
                if (this.debug) {
        ...
        isCameraMovementAcceptable() {
            ...
            if (this.debug && (!positionAcceptable || !rotationAcceptable)) {
                print(`Camera movement too fast - Position: ${positionSpeed.toFixed(1)} cm/s (max: ${this.maxCameraMovementSpeed}), Rotation: ${rotationSpeed.toFixed(1)} deg/s (max: ${this.maxCameraRotationSpeed})`);
            }
         ...
         */
        shouldUpdateDetectionPosition(detectionIndex, newPosition) {
            ...
            if (this.debug && !shouldUpdate) {
                print(`Detection ${detectionIndex}: Distance changed ${distanceChanged.toFixed(1)}cm (threshold: ${this.positionUpdateThreshold}cm)`);
            }
            return shouldUpdate;
        }
        /**
         ...
         */
        shouldUpdateSeamlessDetection(detectionIndex, newCenter, newVertices) {
            ...
            if (this.debug && !updateNeeded) {
                print(`Detection ${detectionIndex}: Center Δ ${centerDistanceChanged.toFixed(1)}cm (threshold: ${this.positionUpdateThreshold}cm), Vertices stable: ${!shouldUpdateVertices}`);
            }
            return {
                updateNeeded,
         ...
         */
        startLerpTransition(detectionIndex, newPosition, newVertices, newRotation = null) {
            if (!this.enableStaticScene ||
        ...
        updateLerpTransitions() {
            ...
            this.activeLerps.forEach((lerpState, detectionIndex) => {
                ...
                const easedProgress = (0, SpatializerUtils_1.easeOutCubic)(progress);
                // Lerp position
                const currentPos = (0, SpatializerUtils_1.lerpVec3)(lerpState.startPosition, lerpState.targetPosition, easedProgress);
                ...
                if (this.enableRotationLerp && lerpState.targetRotation) {
                    ...
                    const easedRotationProgress = (0, SpatializerUtils_1.easeOutCubic)(rotationProgress);
                ...
                if (lerpState.startVertices.length > 0 &&
                    ...
                    const easedVertexProgress = (0, SpatializerUtils_1.easeOutCubic)(vertexProgress);
                    const interpolatedVertices = [];
                    for (let i = 0; i < lerpState.startVertices.length; i++) {
                        const interpolatedVertex = (0, SpatializerUtils_1.lerpVec3)(lerpState.startVertices[i], lerpState.targetVertices[i], easedVertexProgress);
         ...
         */
        getCurrentDetectionVertices(instance) {
            const detectionContainer = instance.getComponent(DetectionContainer_1.DetectionContainer.getTypeName());
         ...
         */
        updateDetectionVertices(instance, vertices) {
            const detectionContainer = instance.getComponent(DetectionContainer_1.DetectionContainer.getTypeName());
            if (!detectionContainer || !detectionContainer.polylinePoints) {
                return;
            }
    ...
    __setFunctionName(_classThis, "DepthSpatializer");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        DepthSpatializer = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return DepthSpatializer = _classThis;
})();
exports.DepthSpatializer = DepthSpatializer;
//# sourceMappingURL=DepthCacheSpatializer.js.map